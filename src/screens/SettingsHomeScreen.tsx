...
export default function SettingsHomeScreen() {
  const M = useMessages()
  const OPTIONS = [
    { value: 'feed' as const, label: M.settings.homeFeedOption, desc: M.settings.homeFeedOptionDesc },
    { value: 'record' as const, label: M.settings.homeRecordOption, desc: M.settings.homeRecordOptionDesc },
  ]
  const goBack = useAppStore((s) => s.goBack)
  const homeScreenIsRecord = useAppStore((s) => s.homeScreenIsRecord)
  const acceptHomePrompt = useAppStore((s) => s.acceptHomePrompt)
  const dismissHomePrompt = useAppStore((s) => s.dismissHomePrompt)

  const current = homeScreenIsRecord ? 'record' : 'feed'

  const select = (v: 'feed' | 'record') => {
    if (v === 'record') acceptHomePrompt()
    else dismissHomePrompt()
  }
  ...
  return (
    <div>
      <div style={{ padding: 'calc(14px + env(safe-area-inset-top)) 20px 14px', background: '#FFFFFF', borderBottom: '1px solid #EBEBEB', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <span style={{ fontSize: 15, fontWeight: 700, color: '#111111' }}>{M.settings.homeScreen}</span>
      </div>

      <div style={{ padding: '8px 20px' }}>
        <p style={{ margin: '16px 0 12px', fontSize: 12, color: '#AAAAAA', fontWeight: 300 }}>{M.settings.homeScreenGuide}</p>
        {OPTIONS.map((opt) => {
