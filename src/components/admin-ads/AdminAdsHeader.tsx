...
interface AdminAdsHeaderProps {
  goBack: () => void
}

export function AdminAdsHeader({ goBack }: AdminAdsHeaderProps) {
  const M = useMessages()
  return (
    <div style={{ padding: 'calc(14px + env(safe-area-inset-top)) 20px 14px', background: '#FFFFFF', borderBottom: '1px solid #EBEBEB', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 10 }}>
      ...
      <span style={{ fontSize: 15, fontWeight: 700, color: '#111111' }}>{M.adminAds.title}</span>
    </div>
  )
}
